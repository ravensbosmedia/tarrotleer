...
export const TarotCard: React.FC<Props> = ({
  ...
  return (
    <div
      className={`relative w-20 h-32 md:w-24 md:h-36 rounded-lg shadow-lg cursor-pointer
    ...
    >
      {/* roterende kaartinhoud */}
      <div
        className={`w-full h-full rounded-lg overflow-hidden transition-transform duration-500
      ...
      {/* selectiebadge staat buiten de rotatie */}
      {selected && selectionNumber !== undefined && (
        <div className="absolute top-1 right-1 w-5 h-5 bg-purple-600 rounded-full flex items-center justify-center text-white text-xs font-bold z-10">
          {selectionNumber}
        </div>
      )}
      ...
      {isReversed && !faceDown && (
        <div className="absolute bottom-1 left-0 right-0 flex justify-center z-10">
          <span className="bg-black/50 text-white text-[9px] px-1 rounded">omgekeerd</span>
        </div>
      )}
    </div>
  );
